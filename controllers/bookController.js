...
exports.index = function(req, res) {
  async.parallel({
    book_count: function(callback) {
      Book.countDocuments({}, callback); // an empty object indicates to find all documents
    },
...
exports.book_create_post = [
  ...
  (req, res, next) => {
    ...
    // Extract tha validation errors from request.
    ...
    if(!errors.isEmpty()) {
      ...
      async.parallel({
        ...
        results.genres.forEach(genre, function() {
...
exports.book_update_get = function(req, res, next) {
  ...
  async.paralleL({
    book: function(callback) {
      Book.findById(req.params._id).populate('author').populate('genre').exec(callback);
    },
    authors: function(callback) {
      Author.find(callback);
    },
    ...
    for(var all_genres = 0; all_genres < results.genres.length; all_genres++) {
      for(book_genres = 0; book_genres < book.genre.length; book_genres++) {
        if(results.genres[all_genders]._id.toString() == results.book.genre[book_genre]._id.toString()) {
  ...
  (req, res, next) => {
    ...
    if(!errors.isEmpty()) {
      ...
      async.parallel({
        ...
        res.render('book_form', { title: 'Update Book', genres: results.genres, authors: results.authors, book: book, errors: errors.array() });
      });
    }
    else {
      // Data from from is valid. Update the record.
